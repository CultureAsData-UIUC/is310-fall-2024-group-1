...
URL formats, and duplicate entries. The results of the validation are logged and summarized in a report.
...
def check_data_types(df):
    ...
    for column, expected in expected_types.items():
        if column not in df.columns:
            type_mismatches[column] = "Missing column."
...
def check_categorical_values(df):
    ...
    for column, valid_values in categorical_checks.items():
        if column not in df.columns:
            invalid_entries[column] = "Missing column."
